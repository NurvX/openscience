...
describe("OpenScience landing contract", () => {
  ...
  test("keeps installation on a dedicated platform-aware download page", () => {
    ...
    expect(download).toContain("curl -fsSL https://openscience.sh/install-desktop | bash")
    expect(download).toContain("Copy macOS installer")
    const macPrimaryAction = download.slice(
      download.indexOf("<Copy command={MAC} primary />"),
      download.indexOf('<details className="group', download.indexOf("<Copy command={MAC} primary />")),
    )
    expect(macPrimaryAction).toContain("Checksum-verified before install")
    expect(macPrimaryAction).toContain("ad-hoc signed")
    expect(macPrimaryAction).toContain("not Apple-notarized")
    expect(macPrimaryAction).not.toContain("Verified installer")
    const directDownloadStart = download.lastIndexOf("href={`${RELEASE}/${download.file}`}")
    const directDownloadAction = download.slice(directDownloadStart, download.indexOf("</>", directDownloadStart))
    expect(directDownloadAction).toContain("Unsigned Windows installer. Microsoft SmartScreen may show a warning.")
    expect(directDownloadAction).not.toContain("<details")
    expect(download).toContain("OpenScience verifies, installs,")
    expect(download).toContain("Manual disk image")
    expect(download).toContain("Use the installer above to")
    expect(download).toContain("avoid that screen")
    expect(download).not.toContain("Copy the verified installer")
    expect(download.indexOf("Copy macOS installer")).toBeLessThan(download.indexOf("Manual disk image"))
  ...
  test("keeps public Ace copy aligned across the landing page, README, and docs", () => {
    for (const source of [readme, gateway]) {
      ...
      expect(source).toMatch(/\$0 authorization/i)
      expect(source).not.toMatch(/funds only the gap/i)
