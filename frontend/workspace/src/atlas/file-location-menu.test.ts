...
describe("Files sources", () => {
  test("renders accessible source groups without exposing host paths as product identity", () => {
    ...
    expect(host.querySelector("#project-folder-heading")?.textContent).toBe("Project folder")
    expect(host.querySelector("#session-files-heading")?.textContent).toBe("Session files")
    ...
    expect(host.textContent).toContain("Already connected")
