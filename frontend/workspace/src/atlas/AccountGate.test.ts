...
describe("workspace account gate", () => {
  test("requires account connection before the skippable model setup", async () => {
    ...
    expect(gate).toContain("choose the workspace you want to use")
    expect(gate).toContain("same Wallet, members, integrations, and usage rules")
    expect(gate).toContain("Synthetic Sciences API key")
    expect(gate).toContain("Legacy thk_ keys stay valid and map to Personal")
